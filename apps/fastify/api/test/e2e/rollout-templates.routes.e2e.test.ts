...
describe('Rollout templates routes (e2e)', () => {
  ...
  it('GET /api/rollout-templates (paginated)', async () => {
    const res = await client.getRolloutTemplates({ queryParams: { page: 1, pageSize: 10 } });
    expect(res.status).toBe(200);
  });

  it('GET /api/rollout-templates/{id}', async () => {
    const res = await client.getRolloutTemplateById({ pathParams: { id: 'rt-1' } });
    expect(res.status).toBe(200);
  ...
  it('POST /api/rollout-templates/{id}/push', async () => {
    ...
    expect(res.data).toMatchObject({ success: true, jobId: expect.any(String) });
  });
});
