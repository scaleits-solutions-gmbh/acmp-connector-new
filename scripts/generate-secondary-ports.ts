...
const clientQueryRepository: QueryRepositoryDefinition = {
  ...
  description: 'Query repository for reading client data from ACMP MSSQL database.',
  imports: `import {
  AcmpClientReadModel,
...
const clientHardDriveQueryRepository: QueryRepositoryDefinition = {
  ...
  description: 'Query repository for reading client hard drive data from ACMP MSSQL database.',
  imports: `import {
...
const clientNetworkCardQueryRepository: QueryRepositoryDefinition = {
  ...
  description: 'Query repository for reading client network card data from ACMP MSSQL database.',
  imports: `import {
...
const clientInstalledSoftwareQueryRepository: QueryRepositoryDefinition = {
  ...
  description: 'Query repository for reading client installed software data from ACMP MSSQL database.',
  imports: `import {
...
const jobQueryRepository: QueryRepositoryDefinition = {
  ...
  description: 'Query repository for reading job data from ACMP MSSQL database.',
  imports: `import {
  AcmpJobReadModel,
} from '@scaleits-solutions-gmbh/acmp-connector-lib-global-common-kit';
import { PaginationOption } from '@scaleits-solutions-gmbh/org-lib-backend-common-kit';
...
const ticketQueryRepository: QueryRepositoryDefinition = {
  ...
  description: 'Query repository for reading ticket data from ACMP MSSQL database.',
  imports: `import {
...
const assetQueryRepository: QueryRepositoryDefinition = {
  ...
  description: 'Query repository for reading asset data from ACMP MSSQL database.',
  imports: `import {
...
const clientCommandQueryRepository: QueryRepositoryDefinition = {
  ...
  description: 'Query repository for reading client command data from ACMP MSSQL database.',
  imports: `import {
...
const rolloutTemplateQueryRepository: QueryRepositoryDefinition = {
  ...
  description: 'Query repository for reading rollout template data from ACMP MSSQL database.',
  imports: `import {
...
function generateRepositoriesIndex(): string {
  return repositoryGroups.map((g) => `export * from './${g.folder}';`).join('\n') + '\n';
}
...
function main(): void {
  ...
  for (const group of repositoryGroups) {
    ...
    writeFile(path.join(groupPath, 'index.ts'), generateRepositoryFolderIndex(group));
  }

  // Generate repositories/index.ts
  writeFile(path.join(repositoriesPath, 'index.ts'), generateRepositoriesIndex());
  ...
  writeFile(path.join(sicsApiPath, `${sicsApiAdapter.fileName}.ts`), generateSicsApiAdapter());
  ...
  // Summary
  const totalQueryRepos = repositoryGroups.reduce((acc, g) => acc + g.queryRepositories.length, 0);
  const totalWriteRepos = repositoryGroups.reduce((acc, g) => acc + g.writeRepositories.length, 0);
